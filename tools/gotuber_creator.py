...
使い方:
  ...
  python tools/gotuber_creator.py generate-depth --input assets/characters/mychar --batch-size 4
...
batch 推論は --batch-size で調整可能 (default: 1)。RTX 2060 12GB では 1-2 が安定。
"""
...
def _generate_depth_heuristic(
    ...
    """Pillow のみで的確な depth map を生成する (Phase 3.6 heuristic backend)。

    方式:
      1. 元画像を RGBA として読み込み、alpha チャンネルを抽出
      2. RGBA → L (grayscale) に変換
      3. エッジ検出 (FIND_EDGES) で輪郭を抽出
      4. 中心からの距離フィールドを加算 (人物は中心に近いほど手前と仮定)
      5. 0-255 に正規化
      6. alpha=0 のピクセルを深度 0 に確定し、境界にフェザリングを適用
    ...
    # --- 背景モヤ取り: alpha=0 を黒に確定 + 境界フェザリング ---
    import numpy as np

    result_arr = np.array(result, dtype=np.uint8)

    # alpha=0 のピクセルを深度 0 (黒) に強制
    alpha_arr = np.array(alpha, dtype=np.uint8)
    fg_mask = alpha_arr > 0
    result_arr[~fg_mask] = 0

    # 境界フェザリング: alpha チャンネルのエッジにガウシアンブラーを適用
    alpha_mask = alpha.point(lambda x: 255 if x > 0 else 0)
    blurred_mask = alpha_mask.filter(ImageFilter.GaussianBlur(radius=3))
    blurred_arr = np.array(blurred_mask, dtype=np.float32) / 255.0

    feather_mask = blurred_arr > 0.01
    result_arr[feather_mask] = (
        result_arr[feather_mask] * blurred_arr[feather_mask]
    ).astype(np.uint8)
...
def _normalize_depth_alpha_aware(
    depth_np: "np.ndarray",  # type: ignore[name-defined]
    src_img_rgba: "PILImage.Image",  # type: ignore[name-defined]
) -> "np.ndarray":  # type: ignore[name-defined]
    """alpha channel を使った前景アウェアな depth 正規化。

    透明背景や黒背景が min/max を支配しないように、
    alpha > 0 のピクセル（前景）のみで正規化範囲を計算する。
    さらに、alpha=0 のピクセルは depth=0 に確定させ、
    境界ピクセルにはガウシアンブラーでフェザリングを適用する。

    Args:
        depth_np: DA3 が出力した depth 配列 (H, W) float32。奥=大値。
        src_img_rgba: 元画像の RGBA版。alpha チャネル抽出に使う。

    Returns:
        uint8 配列 (H, W)。白(255)=手前 / 黒(0)=奥。
    """
    import numpy as np
    from PIL import Image as _PILImg

    # alpha チャネルを取得して前景マスクを作成
    alpha = np.array(src_img_rgba.split()[-1])  # (H, W), uint8
    fg_mask = alpha > 0

    if fg_mask.any():
        fg_min = float(depth_np[fg_mask].min())
        fg_max = float(depth_np[fg_mask].max())
    else:
        # 前景ピクセルが無い場合（全透過）→ 通常正規化にフォールバック
        fg_min = float(depth_np.min())
        fg_max = float(depth_np.max())

    if fg_max - fg_min < 1e-6:
        return np.full_like(depth_np, 128, dtype=np.uint8)

    # 前景色範囲で正規化し、白=手前 / 黒=奥 に反転
    depth_norm = (depth_np - fg_min) / (fg_max - fg_min)
    depth_u8 = ((1.0 - depth_norm) * 255.0).clip(0, 255).astype(np.uint8)

    # --- 背景モヤ取り: alpha=0 を黒に確定 + 境界フェザリング ---
    # alpha=0 のピクセルは深度 0 (黒) に強制
    depth_u8[~fg_mask] = 0

    # 境界フェザリング: alpha チャンネルのエッジにガウシアンブラーを適用
    # alpha=0 のピクセルは depth=0、alpha>0 の内部はそのまま、
    # 境界数 px は滑らかに線形遷移する
    alpha_img = _PILImg.fromarray(alpha, mode="L")
    # マスク作成: alpha>0 を 255、alpha=0 を 0 に二値化
    alpha_mask = alpha_img.point(lambda x: 255 if x > 0 else 0)
    # ガウシアンブラーで境界を滑らかに（半径 3 px ≒ 境界 6 px 幅）
    blurred_mask = alpha_mask.filter(ImageFilter.GaussianBlur(radius=3))
    blurred_arr = np.array(blurred_mask, dtype=np.float32) / 255.0

    # フェザーマスク > 0 のピクセルにブレンド適用
    # (depth * blurred_mask) で、外側=0、内側=1、境界=中間値になる
    feather_mask = blurred_arr > 0.01  # 1% 以上の不透明度がある箇所のみ
    depth_u8[feather_mask] = (
        depth_u8[feather_mask] * blurred_arr[feather_mask]
    ).astype(np.uint8)
...
def _generate_depth_da3(
    ...
    """Depth Anything v3 で深度推定 depth map を生成する (単一画像)。

    alpha-aware 正規化を使用: 透明背景が depth 正規化に影響しない。

    注意:
      - depth-anything-3 Python パッケージとモデル weights が必要
      - 初回実行時にモデルが自動ダウンロードされる場合がある
      - model は _load_da3_model() で事前読み込み済みであること
      - バッチ処理を使う場合は _generate_depth_da3_batch() を推奨
    """
    errors: list[str] = []
    import numpy as np
    from PIL import Image as PILImage

    # 画像読み込み (RGBA で開いてから RGB を inference 用に分離)
    try:
        src_rgba = PILImage.open(src_path).convert("RGBA")
        img_rgb = src_rgba.convert("RGB")
    except Exception as exc:
        errors.append(f"読み込み失敗: {src_path.name} ({exc})")
        return errors
    ...
    # DA3 inference
    try:
        prediction = model.inference([img_rgb])  # type: ignore[union-attr]
    except Exception as exc:
    ...
    depth_np = prediction.depth[0]  # (H, W) float32, 奥=大値
    img_rgb.close()

    # DA3 出力は often 504x504 等の smaller size。
    # alpha-aware 正規化の前に元画像サイズにリサイズする。
    target_w, target_h = src_rgba.size
    if depth_np.shape[0] != target_h or depth_np.shape[1] != target_w:
        from PIL import Image as _PILImg
        depth_pil = _PILImg.fromarray(depth_np, mode="F")
        depth_pil = depth_pil.resize((target_w, target_h), _PILImg.Resampling.BICUBIC)
        depth_np = np.array(depth_pil, dtype=np.float32)

    # alpha-aware 正規化
    depth_u8 = _normalize_depth_alpha_aware(depth_np, src_rgba)
    src_rgba.close()
...
def _generate_depth_da3_batch(
    model: object,
    src_paths: list[Path],
    dst_paths: list[Path],
) -> list[str]:
    """Depth Anything v3 で深度推定をバッチ実行する (Phase 3.6)。

    複数画像をまとめて DA3 inference に渡し、VRAM 使用量を抑制しつつ
    単一画像あたりのオーバーヘッドを削減する。

    alpha-aware 正規化を全画像に適用する。

    Args:
        model: DA3 モデル (事前読み込み済み)
        src_paths: 元画像パスのリスト
        dst_paths: 出力先パスのリスト (src_paths と 1:1 対応)
    ...
    if len(src_paths) != len(dst_paths):
        return ["src_paths と dst_paths の長さが一致しない"]

    # 画像をまとめて読み込み
    rgb_images: list[PILImage.Image] = []
    rgba_images: list[PILImage.Image] = []
    valid_indices: list[int] = []

    for i, src_path in enumerate(src_paths):
        try:
            rgba = PILImage.open(src_path).convert("RGBA")
    ...
    # DA3 batch inference
    try:
        prediction = model.inference(rgb_images)  # type: ignore[union-attr]
    except Exception as exc:
        errors.append(f"DA3 batch inference 失敗: {exc}")
    ...
    for batch_idx, global_idx in enumerate(valid_indices):
        depth_np = prediction.depth[batch_idx]  # (H, W) float32
        src_rgba = rgba_images[batch_idx]
        dst_path = dst_paths[global_idx]

        # DA3 出力は often 504x504 等の smaller size。
        # alpha-aware 正規化の前に元画像サイズにリサイズする。
        target_w, target_h = src_rgba.size
        if depth_np.shape[0] != target_h or depth_np.shape[1] != target_w:
            depth_pil = PILImage.fromarray(depth_np, mode="F")
            depth_pil = depth_pil.resize(
                (target_w, target_h), PILImage.Resampling.BICUBIC
            )
            depth_np = np.array(depth_pil, dtype=np.float32)

        depth_u8 = _normalize_depth_alpha_aware(depth_np, src_rgba)
        src_rgba.close()
        rgb_images[batch_idx].close()
...
def cmd_generate_depth(args: argparse.Namespace) -> int:
    ...
    バックエンド:
      - depth-anything-v3: Depth Anything v3 で ML 深度推定 (既定)
        - chunked batch 推論: --batch-size ずつまとめて推論し VRAM を安全に使う
        - alpha-aware 正規化: 透明背景が depth 正規化に影響しない
    ...
    # 全処理対象ファイルを収集してから一括処理する
    # (DA3 バッチ推論 + heuristic は 1 枚ずつ)
    all_src: list[Path] = []
    all_dst: list[Path] = []
    ...
    for sheet in sheets:
        ...
        for r in range(ROWS):
            for c in range(COLS):
                ...
                all_src.append(src_path)
                all_dst.append(dst_path)

    # バックエンドに応じて処理
    if backend == "depth-anything-v3" and all_src:
        # chunked batch 推論: batch_size ずつ分割して VRAM を安全に使う
        batch_size = getattr(args, "batch_size", 1)
        for chunk_start in range(0, len(all_src), batch_size):
            chunk_end = min(chunk_start + batch_size, len(all_src))
            chunk_src = all_src[chunk_start:chunk_end]
            chunk_dst = all_dst[chunk_start:chunk_end]
            errs = _generate_depth_da3_batch(da3_model, chunk_src, chunk_dst)
            if errs:
                total_errors.extend(errs)
            else:
                total_generated += len(chunk_src)

    elif backend == "heuristic" and all_src:
        # heuristic は 1 枚ずつ処理
        for src_path, dst_path in zip(all_src, all_dst):
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_gdepth = sub.add_parser(
        "generate-depth",
        help="Morph Renderer 用 depth map を生成 (batch推論 + alpha-aware正規化)",
    )
    ...
    p_gdepth.add_argument(
        ...
        help="DA3 バッチ推論のチャンクサイズ (default: 1)。"
             "VRAM が小さい場合は 4 に減らす。25 で sheet 全枚を一括化也可能。",
    )
