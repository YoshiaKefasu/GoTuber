...
使い方:
  ...
  python tools/gotuber_creator.py generate-depth --input assets/characters/mychar --sheets A --backend heuristic
...
def cmd_generate_depth(args: argparse.Namespace) -> int:
    ...
    バックエンド:
      - heuristic: Pillow のエッジ検出 + 中心距離フィールドで簡易 depth map を生成
...
def main() -> int:
    ...
    p_gdepth.add_argument(
        ...
        help=(
            "depth 生成バックエンド。"
            "heuristic=Pillow エッジ検出+距離フィールド (default)。"
