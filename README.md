...
| Phase 4 | 実装完了 | Morph Renderer: mesh + depth-weighted elastic morph + Tweaks UI + FPS fallback に加えて、4.5 ease tuning と 4.6 Single-sprite Liquify transition でセル切り替えの重なりを抑制 |
...
🔜 **次の予定**: Phase 3 Creator Tools（Phase 3.1 build-a CLI → Phase 3.2 マスク生成）。Phase 4 Morph Renderer（`docs/PHASE4.md`）は 4.6 Single-sprite Liquify transition まで実装完了。Phase 3.6 Depth Map Generator も完了済み（A〜F 全150枚、DA3 GPU 生成済み）。

- プラン: [docs/PLAN.md](docs/PLAN.md) v0.4.9
