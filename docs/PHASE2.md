...
- libzmq 必須 (KASOU: `apt install libzmq3-dev`、Windows: vcpkg or pre-built DLL)
- gcc PATH 必須 (Windows: `C:\gcc\mingw64\bin` を PATH 追加、Phase 1.13a で設定済)
- libzmq 不在環境では:
  - Phase 1 ビルド (`go build ./...` タグなし) は影響なし
  - `-tags camera` ビルドは失敗 (zmq4 + libzmq が見つからない)
  - supervisor_test は supervisor 単体テストのみ pass、L1/L2 統合テストは環境構築後
- テスト実行: libzmq 環境 (KASOU 等) で `go build -tags camera ./... && go test -tags camera ./...`
...
- Phase 2.6 でヒステリシスを別レイヤで被せる (mapper 単体は変更しない、supervisor / mapper の wrapper で実装)
