...
- ランドマークから以下を計算:
  ...
  - EAR (Eye Aspect Ratio): 上まぶた (159, 145) と下まぶた (135, 33) の縦距離 / 横幅
...
- mp_server.py クラッシュ → mp_client の channel close を検知 → fallback
- 顔未検出 1 秒以上 → mouse follow mode に fallback
