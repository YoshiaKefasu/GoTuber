...
> **ステータス**: MediaPipe Face Landmarker 即採用で確定 (2026-06-17 ユーザー判断)
> **最終更新**: 2026-06-17
> **親プラン**: [PLAN.md](./PLAN.md) v0.4.4
...
- **tracker は supervisor loop で自動再起動** (exponential backoff、上限 5s)
- **5 秒以上顔未検出 / 1 秒以上 tracker dead → mouse follow にフォールバック** (見た目の自然さ優先)
...
実装は `internal/camera/capture.go` の `CameraTracker` で、defer recover + cleanupCtx パターン + atomic ベース状態観測 (SentCount / IsRunning / LastErrorAt) を組み合わせて実現 (Phase 2.2 完了、コードレビュー APPROVE)。
...
**Supervisor loop (配信中可用性方針 Section 1.1 参照)**:
- `CameraTracker` (Phase 2.2) と `MPClient` (Phase 2.3) をそれぞれ **supervisor goroutine** で wrap
- supervisor は `IsRunning()` が false になった瞬間を検知して `Start()` を再呼び出し
- **exponential backoff**: 失敗 1s → 2s → 4s → 8s、上限 30s、3 回成功でリセット
- supervisor の panic もメインプロセス影響なし (defer recover)
- `--no-camera` フラグ有効時は supervisor 起動せず (Phase 1 マウスモード永続)
- Tweaks パネルに「Camera Status: Running / Restarting / Down / Disabled」表示 (Phase 2.8 と統合)
...
- デバイス未接続 → Python サーバーが 1 秒以内に detection 0 を publish → GoTuber 側 fallback
- Python プロセス起動失敗 → GoTuber は正常起動 (マウスモードのみ)、status に "camera unavailable"
- mp_server.py クラッシュ → mp_client の channel close を検知 → **supervisor loop が再起動試行** (Section 1.1 配信中可用性方針)
- **MediaPipe クラッシュ / libzmq エラー / OpenCV エラー** → CameraTracker の defer recover が panic を吸収 → supervisor loop が再起動 (exponential backoff)
- 顔未検出 1 秒以上 → mouse follow mode に fallback (tracker dead 含む)
