...
- **NEW（Phase 3）**: Creator Tools で 1 枚のメイン画像から GoTuber 用キャラクター素材作成を支援
...
| **Should (Phase 2+)** | Webcam 頭の方向 + 瞬きトラッキング（Phase 2、MediaPipe 即採用で確定）/ Creator Tools（Phase 3: 1 枚入力 → A 25 枚 → マスク → AI 補完で 150 枚） |
...
| Phase 3 | Python/Pillow 等（Creator Tools 実行時のみ） | — | 本体 runtime 影響なし |

→ `internal/camera/` 配下は `//go:build camera` でガード。**Phase 1 ビルドには Phase 2 依存不要**。

Phase 2.10.8 では、camera build を維持したまま **Toggles で camera runtime を OFF** できるようにする。OFF 時は supervisor / mp_server.py を止め、mouse follow を常時有効に戻す。
...
│   ├── gotuber_creator.py        # Phase 3 Creator Tools（予定）
...
window:
  ...
  always_on_top: true
  click_through: true
  # kill_switch_key / kill_switch_rightclick: Phase 1.14 で削除済み
...
| R7 | Creator Tools の画像生成品質がユーザー環境に依存 | 中 | Phase 3 は「1 枚入力 → A 25 枚 → マスク生成 → AI 補完」の支援に絞る。AI の画風保証は非ゴールに置く |
...
| R10 | Creator Tools のマスク自動生成が顔パーツ位置ズレに弱い | 中 | 初期は赤マスク review PNG を必須にし、ユーザー手修正を前提にする。完全自動 150 枚生成は post-release 拡張扱い |
...
| R15 | Morph Renderer の mesh/depth 処理で FPS が落ちる | 中 | Phase 4.4 で morph mesh cache、depth map 読み込み skip、FPS 低下時 auto fallback、Morph ON/OFF を実装済み。depth map が無い場合は通常 mesh fallback。 |
