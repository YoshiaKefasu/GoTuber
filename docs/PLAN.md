...
- Phase 2: Python 3.9+ + mediapipe 0.10.14+ + pyzmq 26.0+（`tools/requirements-mp.txt` 参照、Go 側ビルドタグ `camera` のみ）
...
| カメラ入力（Phase 2） | `mediapipe (Python)` + `github.com/pebbe/zmq4` (Go) | mediapipe 0.10.14+, pyzmq 26.0+, zmq4 latest | **2026-06-17 確定**: MediaPipe Face Landmarker Tasks API を Python サイドカー構成で採用。YuNet は不採用 (瞬き EAR が別モデルになり結局 MediaPipe と大差ないため) |
...
| Phase 2 | + zmq4 (Go) + mediapipe (Python サイドカー) | `-tags camera` | Go 側に mediapipe / opencv 依存なし、Python サイドカー起動失敗時は Phase 1 マウスモードで graceful degradation |
...
│   │   ├── capture.go           # ZeroMQ SUB port 5555 (frame consumer)
│   │   ├── mpclient.go          # ZeroMQ SUB port 5556 (detection consumer)
...
│   ├── dev.ps1
│   ├── dev.sh
│   # Phase 2: tools/setup-mp.{ps1,sh} で Python 3 + mediapipe + pyzmq をインストール（Phase 1 公開リリース後に追加）
├── go.mod
├── go.sum
├── README.md
└── LICENSE
...
[入力ソース]
  ...
  └─ カメラ (mp_server.py Python プロセス → ZeroMQ SUB → camera.MPClient + camera.Mapper)  [Phase 2]
...
| 2 | Camera 入力 | 2〜3 週 | **確定 (2026-06-17)**: MediaPipe Face Landmarker 即採用 (Python サイドカー + ZeroMQ)。Phase 1 ビルドサイズ不変 |
...
| Phase 2: ZeroMQ | pyzmq 26.0+ (Python) / pebbe/zmq4 latest (Go) | PUB/SUB IPC |
...
#### Phase 2（zmq4 込み、gocv 不要）

```bash
# Go バイナリビルド (Phase 1 と同じ手順、-tags camera で internal/camera/ を有効化)
CGO_ENABLED=1 go build -tags camera -ldflags "-s -w" \
  -o bin/gotuber ./cmd/gotuber
...
| R6 | Python サイドカー依存追加 | 低 | GoTuber.exe は Go バイナリのみで変化なし、Phase 2 利用時に mediapipe/opencv-python/pyzmq が必要 (~125MB) |
...
- **実行時致命的**（mp_server.py クラッシュ / ZeroMQ ソケット切断 等）: ウィンドウ閉じる X ボタンで安全停止 + ログ（Phase 1.14 で仕様確定）、起動失敗時は graceful degradation で Phase 1 マウスモードへフォールバック
...
- [x] **Q8**: Phase 2 顔モデル
  - **2026-06-17 確定**: **MediaPipe Face Landmarker (Tasks API) 即採用**。Python サイドカー構成 (ZeroMQ IPC) で Go 側に mediapipe / opencv 依存なし。YuNet は不採用 (頭の方向だけなら YuNet で十分だが、まばたき EAR も取りたいので 478 landmarks の MediaPipe に統一)。Phase 1 ビルドサイズ・依存は変化なし (GoTuber.exe は Go バイナリのみ、Python は別プロセス)。詳細は [PHASE2.md](./PHASE2.md) §4
...
- pebbe/zmq4 (Phase 2 Go): https://github.com/pebbe/zmq4
- pyzmq (Phase 2 Python): https://github.com/zeromq/pyzmq
