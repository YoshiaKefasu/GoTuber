...
- **実行時致命的**（gocv クラッシュ等）: ウィンドウ閉じる X ボタンで安全停止 + ログ（Phase 1.14 で仕様確定）
...
15. **Phase 1.14** (予定): 終了ショートカット削除 — `Esc` / `Q` キー検出と `killswitch.Install()` の Windows 限定削除 (`runtime.GOOS == "windows"` ガード)。**Unix では `signal.Notify` を維持** (Ctrl+C → graceful 終了)。**Windows では `signal.Notify` 削除** (Ctrl+C は Go runtime デフォルト即終了、推奨は X ボタン)。終了は **ウィンドウの閉じる X ボタン** (全 OS graceful) または **Ctrl+C** (Unix: graceful / Windows: 即終了)。原因: Phase 1.13 visual test で F1 / Esc 押下時にアプリが即終了する致命バグ発覚。**真因は不明** (Ebitengine v2.9.9 ソース確認 → SIGINT ハンドラ存在せず、「競合」説明は否定済み)。サブフェーズ 1.14.0 (真因調査) 〜1.14.6 (1.3 日)。詳細は `docs/PHASE1.md` Section 11
...
*v0.4.4 改訂: 終了ショートカット削除 (Phase 1.14 追加)。Q 確定反映（Q1, Q3, Q4, Q12 確定、Q6 / Q8 保留）。Phase 1 スコープ = マウス追従 + メインマイク Realtime 口パク + 透過。kill switch は Ebitengine デフォルト + ウィンドウ閉じる X ボタンに統一 (Phase 1.14 で実装)。Phase 1.1 着手条件 = Go 1.25 確認 + リポジトリ owner 名確定。*
