...
| `src/character-config.js` | `internal/character/config.go` + YAML | `basePath`, `ext`, `rows`, `cols`, `sheets` を struct に |
| `public/slices2/{A-F}/r{row}c{col}.{ext}` | `internal/character/atlas.go` | 同じパスをそのまま読める |
| `src/tweaks-panel.jsx` | `internal/tweaks/panel.go` | ebitenui で書き直し |
| `tools/slice_character_sheets.py` | **GoTuber にも同梱**（`tools/`） | 新規ユーザー用。MIT、attribution 付与 |
| `Zen Maru Gothic` (Google Fonts CDN) | `assets/fonts/NotoSansCJKjp-Regular.otf` を `go:embed` | オフライン動作可能 |
...
│   ├── requirements.txt          # Pillow, numpy 等
...
- `base_path` を `filepath.Abs` + `filepath.Clean` で解決済み確認（シンボリックリンクは OS 既定で追跡）
...
| 項目 | 想定 |
|---|---|
| 150 フレーム全展開（最悪ケース） | 486 MB (150 × 900×900 × 4 byte RGBA) |
